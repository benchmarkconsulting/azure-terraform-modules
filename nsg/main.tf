...
resource "azurerm_network_security_rule" "predefined_rules" {
  ...
  direction                   = element(var.predefined_rules[lookup(var.predefined_rules[count.index], "name")], 0)
  access                      = element(var.predefined_rules[lookup(var.predefined_rules[count.index], "name")], 1)
  protocol                    = element(var.predefined_rules[lookup(var.predefined_rules[count.index], "name")], 2)
  source_port_ranges          = split(",", replace(  lookup(var.predefined_rules[count.index], "source_port_range", "*" )  ,  "*" , "0-65535" ) )
  destination_port_range      = element(var.predefined_rules[lookup(var.predefined_rules[count.index], "name")], 4)
  description                 = element(var.predefined_rules[lookup(var.predefined_rules[count.index], "name")], 5)
